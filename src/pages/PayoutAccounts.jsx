...
export default function PayoutAccounts({ onToggleSidebar }) {
  ...
  return (
    <div className="w-full min-h-screen relative">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-10 lg:py-16 space-y-8">
        <header className="mb-2">
          ...
          <h1 className="text-4xl lg:text-5xl font-black text-white tracking-tight mb-3">Configuración de recepción</h1>
        ...
        <section className="grid gap-8 lg:grid-cols-2 items-start">
          <article className="glass-iphone rounded-[2rem] border border-white/10 bg-gradient-to-b from-white/[0.05] to-white/[0.01] p-7 space-y-6 shadow-[0_30px_60px_rgba(0,0,0,0.35)]">
            <div className="flex items-start justify-between gap-4 pb-5 border-b border-white/10">
              ...
              <label className="text-xs text-gray-200 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5">
              ...
              <div className="rounded-2xl border border-white/8 bg-black/25 p-4">
                ...
                <div className="grid gap-4">
                  ...
                  <Field label="Correo de comercio" hint="Contacto principal para liquidaciones y avisos.">
                    <input className={inputClass} value={paypal.merchantEmail} onChange={(e) => setPayPal((prev) => ({ ...prev, merchantEmail: e.target.value }))} placeholder="payments@miempresa.com" />
                  </Field>
              ...
              <div className="rounded-2xl border border-white/8 bg-black/25 p-4">
                ...
                <div className="mt-4">
                  <Field label="Callback URL (opcional)">
                    <input className={inputClass} value={paypal.callbackUrl} onChange={(e) => setPayPal((prev) => ({ ...prev, callbackUrl: e.target.value }))} placeholder="https://mi-erp.com/paypal/callback" />
                  </Field>
                </div>
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Field label="Email soporte pagos">
                      <input className={inputClass} value={card.supportEmail} onChange={(e) => setCard((prev) => ({ ...prev, supportEmail: e.target.value }))} placeholder="soporte-pagos@miempresa.com" />
                    </Field>
                    <Field label="Email chargebacks (opcional)">
                      <input className={inputClass} value={card.chargebackEmail} onChange={(e) => setCard((prev) => ({ ...prev, chargebackEmail: e.target.value }))} placeholder="risk@miempresa.com" />
                    </Field>
                  </div>
              ...
              <div className="rounded-2xl border border-white/8 bg-black/25 p-4">
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    <label className={`text-xs inline-flex items-center gap-2 rounded-lg border px-3 py-2 ${acceptedBrandSet.has('visa') ? 'border-[#e6ff2a]/40 bg-[#e6ff2a]/10 text-[#e6ff2a]' : 'border-white/10 bg-white/5 text-gray-300'}`}>
                      <input type="checkbox" checked={acceptedBrandSet.has('visa')} onChange={() => toggleBrand('visa')} className="accent-[#e6ff2a]" /> Visa
                    </label>
                    <label className={`text-xs inline-flex items-center gap-2 rounded-lg border px-3 py-2 ${acceptedBrandSet.has('mastercard') ? 'border-[#e6ff2a]/40 bg-[#e6ff2a]/10 text-[#e6ff2a]' : 'border-white/10 bg-white/5 text-gray-300'}`}>
