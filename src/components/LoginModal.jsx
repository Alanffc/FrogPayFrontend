...
// --- CAMBIO 1: Agregamos `onAuthSuccess` y `onSwitchToRegister` en los parámetros ---
            ...
            <form onSubmit={handleSubmit} noValidate className="grid gap-6">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full rounded-3xl border ${
                      errors.email ? 'border-red-500 bg-red-500/10 text-red-50' : 'border-white/10 bg-white/5 text-white'
                    } px-4 sm:px-5 py-3 sm:py-3.5 outline-none transition focus:border-[#e6ff2a]/80 focus:ring-2 focus:ring-[#e6ff2a]/15`}
                  />
              ...
              <label className="grid gap-2 text-sm text-gray-300">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className={`w-full rounded-3xl border ${
                      errors.password ? 'border-red-500 bg-red-500/10 text-red-50' : 'border-white/10 bg-white/5 text-white'
                    } px-4 sm:px-5 py-3 sm:py-3.5 pr-12 outline-none transition focus:border-[#e6ff2a]/80 focus:ring-2 focus:ring-[#e6ff2a]/15`}
                  />
                  <button
                    type="button"
                    onClick={() => setPasswordVisible((visible) => !visible)}
                    className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 inline-flex items-center justify-center text-[#e6ff2a] transition hover:text-[#b7f758]"
                  >
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between mt-4">
                <div className="text-sm text-gray-400">
                  ¿No tienes cuenta?{' '}
                  {/* --- CAMBIO 3: Usamos onSwitchToRegister aquí --- */}
                  <button type="button" onClick={onSwitchToRegister} className="text-white font-medium hover:text-[#e6ff2a] transition">
                    Regístrate
                  </button>
                </div>
                
                {/* Observa que el botón no es tipo button, sino que al estar en form hace un submit */}
